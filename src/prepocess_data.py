...
class Encoder(object):
    ...
    def encode(self, json_line):
        data = json_line if isinstance(json_line, dict) else json.loads(json_line)
...
class Partition(object):
    ...
    def process_json_file(self, file_name):
        ...
        # 修改：以 JSON 数组格式加载整个文件
        with open(input_file_name, 'r', encoding='utf-8') as fin:
            data = json.load(fin)  # 加载整个 JSON 数组
        ...
        # 编码每个文档（逐个 JSON 对象）
        encoded_docs = pool.imap(encoder.encode, data, 32)

        level = "document"
        if self.args.split_sentences:
            level = "sentence"

        # 设置输出路径前缀，分别为 .bin 和 .idx 文件生成不同路径
        output_prefix_bin = os.path.join("test_Repositories/bin", os.path.basename(output_prefix))
        output_prefix_idx = os.path.join("test_Repositories/idx", os.path.basename(output_prefix))
